...
import { dbBooksCategories } from "../database/books-categories-db.js";
...
router.get("/books/getAll/:bookId", (req, res) => {
  try {
    ...
    for (let i = 0; i < filteredBook.length; i++) {
      ...
      const currentCategoryName = dbCategories.find(
...
router.get("/categories/getAll/:categoryId", (req, res) => {
  try {
    ...
    const filteredCategories = dbBooksCategories.filter(
